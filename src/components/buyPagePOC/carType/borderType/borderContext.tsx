...
import { formFields as vehicleFormFields } from "./vehicleForm/vehicleFormTypes";
...
const BorderContextProvider = ({ children }: { children: JSX.Element }) => {
  ...
  const getPremium = async () => {
    const body = {
      data: formData,
      productSiteId: PRODUCT_INFO.BORDER.productSiteId,
    };
  ...
  useEffect(() => {
    if (hasMounted.current) {
      if (formData.category.value && formData.durationId.value) {
        getPremium();
      } else {
        removePremium();
      }
  ...
  }, [formData.durationId.value, formData.category.value]);
  // provider logic here
  return (
    <borderContext.Provider value={{ formData: formData, setFormData: setFormData }}>
...
export { borderContext, BorderContextProvider };
