...
import { formFields as dataSharingFields } from "../../dataSharing/dataSharing";
...
const PrivateHealthContextProvider = ({ children }: { children: JSX.Element }) => {
  ...
  const getPremium = async () => {
    const body = {
      data: formData,
      productSiteId: PRODUCT_INFO.PRIVATEHEALTH.productSiteId,
    };
  ...
  return (
    <privateHealthContext.Provider value={{ formData: formData, setFormData: setFormData }}>
      ...
      {alerter.render}
      {children}
    </privateHealthContext.Provider>
  );
};

export { privateHealthContext, PrivateHealthContextProvider };
