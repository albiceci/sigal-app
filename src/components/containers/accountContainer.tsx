...
export const AccountContainer = ({
  ...
  return (
    ...
    <div className="w-[100vw] max-w-[100vw] min-h-[650px] h-[100dvh] flex flex-col items-center justify-center bgGradientCustom pb-[50px] lg:pb-0">
      <div className="w-[90%] md:w-[85%] lg:w-[70%] h-[85%] md:h-[80%] mt-[70px] flex gap-1 lg:gap-0">
        <div
        ...
        <div
          className={`bg-white flex flex-col rounded-md md:flex-grow border lg:border-0 absolute md:static w-full h-full top-0 left-0 z-[100] md:z-[1] md:translate-x-[0] transition-transform duration-500 overflow-hidden ${
            activeTab === null ? "translate-x-[100%]" : "translate-x-[0]"
          }`}
        >
          <div className="px-6 h-14 text-primary font-bold">
            <div className="border-b h-full flex items-center border-primary text-lg gap-4 py-4">
              ...
              <div>
                {tabData.filter((tab) => tab.paramKey === activeTab)[0]?.name}
              </div>
            </div>
          </div>
          <div className="h-auto py-3 my-3 px-9 overflow-auto">{children}</div>
        </div>
      </div>
    </div>
  );
