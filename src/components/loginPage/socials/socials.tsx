...
export const Socials = ({ type, onSuccess }: { type: "login" | "register"; onSuccess: () => void }) => {
  ...
  const onSocialsSuccess = async ({ provider, data }: IResolveParams) => {
    ...
    const jsonData = await customFetch(`/user/socials/${provider}/${type}`, {
      method: "POST",
      body: body,
      headers: {
...
const SocialsButton = ({ type, children }: { type: "google" | "facebook"; children: React.ReactNode }) => {
  const socialsLogo = {
    ...
    google: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="-0.5 0 48 48" version="1.1">
  ...
  return (
    <div className={`flex p-2 rounded w-[250px] cursor-pointer ${containerStyle[type]}`}>
      <div className="flex gap-3 items-center justify-center">
        <div className="w-[20px]">{socialsLogo[type]}</div>
        <div className={`${textStyle[type]}`}>{children}</div>
      </div>
    </div>
  );
};
