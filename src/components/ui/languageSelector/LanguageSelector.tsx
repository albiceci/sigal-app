...
import { SelectInput } from "../form/inputs/selectInput/selectInput";
  ...
  return (
    <div className="w-fit h-fit">
      <div
        ...
        className={`px-2 py-[2px] flex items-center justify-center cursor-pointer border rounded-md ${
          theme === "black" ? "text-presetgray border-presetgray" : "text-white border-white"
        } ${type === "min" ? "pr-0 sm:pr-2 sm:gap-1" : "gap-1"}`}
      >
        <div className={`w-[15px] mr-1 ${type === "min" && "hidden sm:block"}`}>{globeSVG}</div>
