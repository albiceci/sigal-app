  ...
  return (
    <div
      className={`flex items-center justify-center absolute left-1/2 h-full -translate-x-2/4 w-[150px] ${
        scrollPosition >= 50 || !isMovable ? "lg:w-[150px]" : "lg:w-[175px]"
      }`}
    >
      <Link
        className={`w-full self-center transition-transform duration-[.2s]  ${
          scrollPosition >= 50 || !isMovable
            ? "translate-y-0 scale-[.8]"
            : "translate-y-8 scale-[1]"
        }`}
        to="/"
      >
        {theme === "white" ? <LogoPrimary /> : <LogoWhite />}
      </Link>
    </div>
  );
};
